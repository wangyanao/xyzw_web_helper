...
const LINEUPS_API_BASE = "/api/lineups";
...
const loadLineupsFromLocal = () => {
  try {
    ...
    console.error("加载本地缓存阵容失败:", e);
    return [];
  }
};

// 从 token 字符串中提取 roleId
const extractRoleIdFromToken = (tokenStr) => {
  try {
    if (!tokenStr) return null;
    // token 可能是 Base64 或 JSON 字符串
    let tokenObj;
...
const findMatchingLineup = (lineups, target) => {
  if (!Array.isArray(lineups) || !target) return null;
  if (target.id) {
    const byId = lineups.find((l) => l?.id === target.id);
    if (byId) return byId;
  }
  return (
    lineups.find(
      (l) =>
        l?.name === target.name &&
        Number(l?.teamId) === Number(target.teamId) &&
        Number(l?.savedAt || 0) === Number(target.savedAt || 0),
    ) || null
  );
};
...
const resolveLineupForApply = async (targetLineup) => {
  ...
  try {
    ...
    console.warn("应用阵容时读取后端失败，回退本地:", e);
  }
...
const saveLineupsToStorage = async () => {
  ...
  try {
    ...
    console.error("保存阵容到后端失败:", e);
    message.warning("后端保存失败，已保存在本地浏览器");
  }
};
...
const applyLineup = async (lineup) => {
  ...
  const sourceLineup = await resolveLineupForApply(lineup);

  if (!sourceLineup || !Array.isArray(sourceLineup.heroes) || sourceLineup.heroes.length === 0) {
    message.error("要应用的阵容不存在或已损坏");
    lineup.applying = false;
    state.value.isRunning = false;
    return;
  }

  if (Number(sourceLineup.teamId) !== currentTeamId.value) {
    message.warning(
      `此阵容仅适用于阵容槽位 ${sourceLineup.teamId}，当前槽位为 ${currentTeamId.value}`,
    );
    lineup.applying = false;
    state.value.isRunning = false;
    return;
  ...
  try {
    const targetHeroes = [...sourceLineup.heroes];
    ...
    if (
      sourceLineup.legionResearch &&
      Object.keys(sourceLineup.legionResearch).length > 0
    ) {
      ...
      if (currentWeaponId !== sourceLineup.weaponId) {
        try {
          await sendMessageWithRetry(
            ...
            {
              weaponId: sourceLineup.weaponId,
            },
          );
          message.success(
            `玩具已切换为: ${weapon[sourceLineup.weaponId] || sourceLineup.weaponId}`,
          );
        } catch (err) {}
        await delay(COMMAND_DELAY);
      }
...
// 监听selectedToken变化，自动加载该Token的已保存阵容
watch(
